...
/* Unique id. Date.now() alone collides when two records are created in the
   same millisecond, which silently merges them — always add entropy. */
const uid = prefix => (globalThis.crypto?.randomUUID
...
const DEFAULTS = {
  ...
  notes: [],               // [{id,branch,title,body,source,star,created,updated}]
...
let ui = { view: 'today', calMonth: null, calYear: null, selDay: null, sylFilter: 'all', sylQuery: '',
           noteBranch: 'all', noteQuery: '', noteStar: false, editing: null,
...
function render() {
  ...
  ({ today: renderToday, calendar: renderCalendar, syllabus: renderSyllabus,
     notes: renderNotes, stats: renderStats, settings: renderSettings })[ui.view]();
}
...
function renderChrome() {
  ...
  const titles = { today: 'Өнөөдөр', calendar: 'Хуанли', syllabus: 'Хуулийн жагсаалт',
                   notes: 'Тэмдэглэл', stats: 'Статистик', settings: 'Тохиргоо' };
...
const branchName = k => (BRANCHES.find(b => b.key === k) || {}).name || 'Бусад';
const branchTone = { crim: ['#fde8f0', '#a3134c'], civ: ['#eef1fe', '#5a63f0'], adm: ['#e2f6f6', '#0c7070'] };

function filteredNotes() {
  const q = ui.noteQuery.toLowerCase();
  return S.notes
    .filter(n => ui.noteBranch === 'all' || n.branch === ui.noteBranch)
    .filter(n => !ui.noteStar || n.star)
...
function renderNotes() {
  ...
  const starred = S.notes.filter(n => n.star).length;
  $('#notesSummary').textContent = total
    ? `${total} тэмдэглэл · ${starred} онцолсон`
    : 'хараахан хоосон';

  // category tabs
  const counts = { all: total };
  for (const b of BRANCHES) counts[b.key] = S.notes.filter(n => n.branch === b.key).length;
  $('#noteTabs').innerHTML = [['all', 'Бүгд'], ...BRANCHES.map(b => [b.key, b.name])]
    .map(([k, label]) => `<button class="ntab ${ui.noteBranch === k ? 'on' : ''}" data-nb="${k}">
        ${esc(label)}<span class="ntab-n">${counts[k] || 0}</span></button>`).join('');
  $$('[data-nb]').forEach(b => b.addEventListener('click', () => { ui.noteBranch = b.dataset.nb; renderNotes(); }));

  $('#noteStar').classList.toggle('btn-primary', ui.noteStar);
  $('#noteSearch').value = ui.noteQuery;
  ...
  const host = $('#noteList');
  if (!list.length) {
    host.innerHTML = emptyBox(total ? '🔍' : '📝', total
      ? 'Энэ шүүлтэнд тохирох тэмдэглэл алга.'
      : 'Эхний тэмдэглэлээ нэмээрэй — шүүхийн шийдвэр, жишиг хариултын бүтэц, санасан дүрэм.');
    return;
  }
  host.innerHTML = list.map(n => {
    const [bg, fg] = branchTone[n.branch] || ['#f0f1f7', '#5a6079'];
    const preview = n.body.replace(/\s+/g, ' ').slice(0, 160);
    return `<div class="note" data-nid="${n.id}">
      <div class="note-head">
        <span class="note-badge" style="background:${bg};color:${fg}">${esc(branchName(n.branch))}</span>
        <span class="note-title">${esc(n.title)}</span>
        <button class="note-star ${n.star ? 'on' : ''}" data-nstar="${n.id}" aria-label="Онцлох">★</button>
      </div>
      ${n.source ? `<div class="note-source">${esc(n.source)}</div>` : ''}
      <div class="note-preview">${esc(preview)}${n.body.length > 160 ? '…' : ''}</div>
      <div class="note-foot">
        <span class="note-date">${n.updated ? fmtShort(iso(new Date(n.updated))) : ''}</span>
        <button class="note-act" data-nedit="${n.id}">Засах</button>
        <button class="note-act danger" data-ndel="${n.id}">Устгах</button>
      </div>
    </div>`;
  }).join('');

  $$('[data-nstar]').forEach(b => b.addEventListener('click', () => {
    const n = S.notes.find(x => x.id === b.dataset.nstar);
    if (n) { n.star = !n.star; n.updated = Date.now(); save(); renderNotes(); }
  }));
  $$('[data-nedit]').forEach(b => b.addEventListener('click', () => {
    ui.editing = b.dataset.nedit; renderNotes();
    $('#noteEditor').scrollIntoView({ block: 'nearest' });
  }));
  $$('[data-ndel]').forEach(b => b.addEventListener('click', () => {
    const n = S.notes.find(x => x.id === b.dataset.ndel);
    if (!n || !confirm(`"${n.title}" устгах уу?`)) return;
    S.notes = S.notes.filter(x => x.id !== b.dataset.ndel);
    save(); renderNotes(); toast('Устгалаа');
  }));
}

function renderNoteEditor() {
  const host = $('#noteEditor');
  if (!ui.editing) { host.innerHTML = ''; return; }
  const isNew = ui.editing === 'new';
  const n = isNew
    ? { id: null, branch: ui.noteBranch === 'all' ? 'crim' : ui.noteBranch, title: '', body: '', source: '' }
    : S.notes.find(x => x.id === ui.editing);
  if (!n) { ui.editing = null; host.innerHTML = ''; return; }

  host.innerHTML = `<div class="note-edit">
    <div class="ne-row">
      <input class="input" id="neTitle" placeholder="Гарчиг — жишээ нь: 10.1 vs 10.6 ялгаа" value="${esc(n.title)}" maxlength="140">
      <select class="input select" id="neBranch">
        ${BRANCHES.map(b => `<option value="${b.key}" ${b.key === n.branch ? 'selected' : ''}>${esc(b.name)}</option>`).join('')}
      </select>
    </div>
    <input class="input" id="neSource" placeholder="Эх сурвалж — тогтоолын дугаар, холбоос, номын хуудас" value="${esc(n.source || '')}" maxlength="200">
    <textarea class="input" id="neBody" rows="9" placeholder="Тэмдэглэл…">${esc(n.body)}</textarea>
    <div class="btn-row">
      <button class="btn btn-primary" id="neSave">Хадгалах</button>
      <button class="btn" id="neCancel">Болих</button>
    </div>
  </div>`;
  ...
  $('#neSave').addEventListener('click', () => {
    ...
    if (!title && !body) { toast('Гарчиг эсвэл агуулга шаардлагатай'); return; }
    const now = Date.now();
    if (isNew) {
      S.notes.push({ id: uid('n'), branch: $('#neBranch').value,
        title: title || '(гарчиггүй)', body, source: $('#neSource').value.trim(),
        star: false, created: now, updated: now });
    } else {
      Object.assign(n, { branch: $('#neBranch').value, title: title || '(гарчиггүй)',
        body, source: $('#neSource').value.trim(), updated: now });
    }
    ui.editing = null; save(); renderNotes(); toast('Хадгаллаа');
  });
  $('#neCancel').addEventListener('click', () => { ui.editing = null; renderNotes(); });
  $('#neTitle').focus();
}

/* ---- review mode: flip through notes one at a time ---- */
function openReview() {
  const list = filteredNotes();
  if (!list.length) { toast('Давтах тэмдэглэл алга'); return; }
  ui.reviewList = list; ui.reviewIdx = 0;
...
function stepReview(d) {
  if (!ui.reviewList.length) return;
  ui.reviewIdx = (ui.reviewIdx + d + ui.reviewList.length) % ui.reviewList.length;
  paintReview();
}
function paintReview() {
  const n = ui.reviewList[ui.reviewIdx];
  if (!n) return closeReview();
  const [bg, fg] = branchTone[n.branch] || ['#f0f1f7', '#5a6079'];
  $('#reviewCount').textContent = `${ui.reviewIdx + 1} / ${ui.reviewList.length}`;
  $('#reviewStar').classList.toggle('btn-primary', !!n.star);
  $('#reviewBody').innerHTML = `
    <span class="note-badge" style="background:${bg};color:${fg}">${esc(branchName(n.branch))}</span>
    <h2 class="review-title">${esc(n.title)}</h2>
    ${n.source ? `<div class="note-source">${esc(n.source)}</div>` : ''}
    <div class="review-text">${esc(n.body).replace(/\n/g, '<br>')}</div>`;
}

function exportNotes() {
  if (!S.notes.length) { toast('Тэмдэглэл алга'); return; }
  const lines = ['# BarTrack — Тэмдэглэл', '', `_${fmtDate(todayISO())}_`, ''];
  for (const b of BRANCHES) {
    const list = S.notes.filter(n => n.branch === b.key).sort((x, y) => (x.created || 0) - (y.created || 0));
    if (!list.length) continue;
    lines.push(`## ${b.name}`, '');
    for (const n of list) {
      lines.push(`### ${n.star ? '★ ' : ''}${n.title}`);
      if (n.source) lines.push(`*${n.source}*`, '');
      lines.push(n.body, '');
    }
  }
  download(`bartrack-temdeglel-${todayISO()}.md`, lines.join('\n'), 'text/markdown;charset=utf-8');
  toast('Тэмдэглэл татагдлаа');
}
...
function init() {
  ...
  $('#noteSearch').addEventListener('input', e => { ui.noteQuery = e.target.value; renderNotes(); });
  $('#noteStar').addEventListener('click', () => { ui.noteStar = !ui.noteStar; renderNotes(); });
  $('#noteNew').addEventListener('click', () => { ui.editing = 'new'; renderNotes(); });
  $('#noteReview').addEventListener('click', openReview);
  $('#noteExport').addEventListener('click', exportNotes);
  ...
  $('#reviewStar').addEventListener('click', () => {
    const n = ui.reviewList[ui.reviewIdx];
    if (n) { n.star = !n.star; n.updated = Date.now(); save(); paintReview(); }
  });
    ...
    r.onload = () => {
      try {
        ...
        S = { settings: { ...DEFAULTS.settings, ...d.settings }, log: d.log,
              custom: d.custom || {}, notes: d.notes || [],
              lastNotified: d.lastNotified || null };
